...
FILE_MARKER_DIR = "./file_markers_prediction"
...
def main(
    ...
    with open(os.path.join(
            FILE_MARKER_DIR, type + "trainSet_seq2seq_" + str(clip_len) + "s.txt"), "r") as f:
        train_str = f.readlines()
    train_tuples = [curr_str.strip('\n').split(',') for curr_str in train_str]

    with open(os.path.join(
            FILE_MARKER_DIR, type + "devSet_seq2seq_" + str(clip_len) + "s.txt"), "r") as f:
        dev_str = f.readlines()
    dev_tuples = [curr_str.strip('\n').split(',') for curr_str in dev_str]

    with open(os.path.join(
            FILE_MARKER_DIR, type + "testSet_seq2seq_" + str(clip_len) + "s.txt"), "r") as f:
