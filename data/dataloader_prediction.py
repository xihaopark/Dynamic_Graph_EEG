...
FILEMARKER_DIR = Path(repo_paths).joinpath('data/file_markers_prediction')
...
class SeizureDataset(Dataset):
    def __init__(
        ...
        file_contents = open(file_marker, "r")
        file_str.extend(file_contents.readlines())


        np.random.shuffle(file_str)
        ...
        if split == "train":
            ...
            labels = [tup[1] for tup in self.file_tuples]
            label_counts = Counter(labels)

            # Get the count of the less frequent label
            min_count = min(label_counts.values())

            # Randomly select min_count samples for each label
            balanced_tuples = []
            for label in [0, 1]:
                label_tuples = [tup for tup in self.file_tuples if tup[1] == label]
                balanced_tuples.extend(random.sample(label_tuples, min_count))

            # Replace the original list with the balanced data
            self.file_tuples = balanced_tuples

        # Print statistics about the balanced dataset
        print(f'Number of clips in {split}: {len(self.file_tuples)}')
        print(f'Number of Normal: {sum(1 for tup in self.file_tuples if tup[1] == 0)}')
        print(f'Number of Pre-seizure: {sum(1 for tup in self.file_tuples if tup[1] == 1)}')
    ...
    def __getitem__(self, idx):
        ...
        os.makedirs(os.path.join("graph_cache_prediction", str(self.max_seq_len), self.filter_type), exist_ok=True)
        cache_file_path = os.path.join("graph_cache_prediction", str(self.max_seq_len), self.filter_type, cache_file_name)
